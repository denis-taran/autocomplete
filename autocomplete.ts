...
export default function autocomplete<T extends AutocompleteItem>(
  ...
  const container: HTMLDivElement =
    settings.container || doc.createElement("div");
  const preventSubmit: PreventSubmit =
    settings.preventSubmit || PreventSubmit.Never;
  ...
  function handleArrowAndEscapeKeys(
    ev: KeyboardEvent,
  ...
  function fetch(trigger: EventTrigger) {
    if (input.value.length >= minLen || trigger === EventTrigger.Focus) {
      ...
      debounceTimer = window.setTimeout(
        ...
        trigger === EventTrigger.Keyboard || trigger === EventTrigger.Mouse
          ? debounceWaitMs
          : 0,
      );
    } else {
  ...
  function destroy() {
    ...
    input.removeEventListener(
      "keyup",
      keyupEventHandler as EventListenerOrEventListenerObject,
    );
    input.removeEventListener(
      "click",
      clickEventHandler as EventListenerOrEventListenerObject,
    );
    input.removeEventListener(
      "keydown",
      keydownEventHandler as EventListenerOrEventListenerObject,
    );
    input.removeEventListener(
      "input",
      inputEventHandler as EventListenerOrEventListenerObject,
    );
